...
export default function ImageToPdfScreen() {
  ...
  const handlePick = (pickedFiles: any[]) => {
    const imageFile = pickedFiles.find((f) => f.name.match(/\.(jpg|jpeg|png)$/i));
    if (imageFile) {
      setFile(imageFile);
    } else {
      Alert.alert('Vui lòng chọn một file hình ảnh.');
    }
  };

  const convertToPdf = () => {
    if (!file) {
      Alert.alert('Vui lòng chọn một file hình ảnh để chuyển đổi.');
      return;
    }

    console.log('Ready to convert:', file);
    Alert.alert('Chức năng chuyển đổi hình ảnh sang PDF chưa xử lý.');
  };

  return (
    <View style={[styles.container, isDarkMode ? styles.darkContainer : styles.lightContainer]}>
      <View style={{ marginTop: 20 }}>
        <HeaderBack title="Chuyển Hình ảnh sang PDF" />
      </View>
